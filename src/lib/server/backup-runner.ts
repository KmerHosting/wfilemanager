...
const worker = process.env.WFILEMANAGER_BACKUP_WORKER || "/usr/local/bin/wfilemanager-backup-worker";
export async function startRemoteBackup(source: unknown, jobId: unknown, signedUrl: unknown) {
  ...
  await access(worker, constants.X_OK).catch(() => { throw new LocalApiError(503, "Backup worker is not installed"); });
