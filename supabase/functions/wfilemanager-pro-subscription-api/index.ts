...
function randomHex(byteLength = 16) { const bytes = new Uint8Array(byteLength); crypto.getRandomValues(bytes); return bytesToHex(bytes).toUpperCase(); }
...
function orderReference(orderType = "new_licence_key") {
  ...
  const prefix = orderType === "renewal" ? "WFM-REN" : orderType === "storage_upgrade" ? "WFM-STO" : "WFM-LIC";
...
function paymentUrlFrom(payload: Record<string, unknown>) {
  return clean(pick(payload, ["pay_url", "payUrl", "payment_url", "paymentUrl", "checkout_url", "checkoutUrl", "redirect_url", "redirectUrl", "url", "link", "data.pay_url", "data.payment_url", "data.checkout_url", "data.redirect_url", "data.url", "data.link"]));
}
function providerRefFrom(payload: Record<string, unknown>) {
  return clean(pick(payload, ["transaction_uuid", "uuid", "reference", "transaction_id", "transactionId", "payment_id", "paymentId", "data.transaction_uuid", "data.uuid", "data.reference", "data.transaction_id", "data.payment_id"]));
}
function invoiceFromPayload(payload: Record<string, unknown>) {
  return clean(pick(payload, ["merchant_invoice_id", "merchantInvoiceId", "idempotency_key", "invoice_id", "invoiceId", "order_reference", "orderReference", "data.merchant_invoice_id", "data.merchantInvoiceId", "data.idempotency_key", "data.invoice_id", "data.order_reference"]));
}
...
async function createCamerPayLink(config: Config, order: Order) {
  ...
  if (["orange_money", "mtn_momo", "stripe", "paypal"].includes(paymentMethod)) body.payment_method = paymentMethod;
...
async function sendLicenceEmail(config: Config, params: { email: string; name: string; orderReference: string; licenceKey: string }) {
  ...
  await sendMail(config, { email: params.email, name, subject: "Your wFileManager Pro licence key", text, html, category: "wfilemanager-pro-licence-key" });
}

async function sendRenewalEmail(config: Config, params: { email: string; name: string; orderReference: string; instanceKey: string; paidUntil: string }) {
  const name = params.name || "Customer";
  ...
  await sendMail(config, { email: params.email, name, subject: "Your wFileManager Pro renewal is confirmed", text, html, category: "wfilemanager-pro-renewal" });
}
...
async function checkout(config: Config, body: Record<string, unknown>) {
  ...
  const orderType = ["renewal", "storage_upgrade"].includes(clean(body.orderType)) ? clean(body.orderType) : "new_licence_key";
  ...
  if (orderType === "renewal") {
    ...
    if (!await customerOwnsInstance(buyerEmail, targetInstanceKey)) return json({ error: "This instance is not linked to your customer account" }, 403);
  }
  ...
  try {
    ...
    return json({ orderReference: reference, orderType, targetInstanceKey, paymentUrl: payment.paymentUrl, amountUsd: config.priceUsd, amountXaf: config.priceXaf, currency: config.currency, status: "payment_pending" });
  } catch (error) {
    await supabase.from("wfilemanager_pro_orders").update({ status: "failed", provider_payload: { error: error instanceof Error ? error.message : "Payment link generation failed" } }).eq("id", order.id);
    throw error;
  }
}
...
async function refreshFromCamerPay(config: Config, order: Order) {
  ...
  if (["activation_sent", "email_failed", "renewal_applied", "upgrade_applied"].includes(String(order.status))) return order;
  ...
  const payload = await response.json().catch(() => ({})) as Record<string, unknown>;
  if (!response.ok) {
    await supabase.from("wfilemanager_pro_orders").update({ webhook_payload: { status_check_error: camerPayError(response.status, payload), status_check_payload: payload } }).eq("id", order.id);
    return order;
  }
  await updatePaymentFromPayload(order, payload);
  const { data: refreshed, error } = await supabase.from("wfilemanager_pro_orders").select("*").eq("id", order.id).single();
  if (error) throw error;
  return refreshed;
}
...
async function reissueFromOldUnclaimedToken(order: Order) {
  ...
  const { data: oldToken, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw error;
  if (!oldToken || oldToken.claimed_at || oldToken.claimed_by_instance_id || oldToken.status === "claimed") return false;
  await supabase.from("wfilemanager_pro_activation_tokens").update({ status: "revoked", updated_at: new Date().toISOString() }).eq("id", oldToken.id);
  await supabase.from("wfilemanager_pro_orders").update({ activation_token_id: null, token_email_sent_at: null, token_email_error: null, status: "paid" }).eq("id", order.id);
  return true;
}
...
async function issueLicenceKeyOnce(config: Config, order: Order) {
  if (order.activation_token_id && !order.license_key_plain) {
    ...
    if (reissued) {
      const { data: reload, error } = await supabase.from("wfilemanager_pro_orders").select("*").eq("id", order.id).single();
      if (error) throw error;
      order = reload;
    }
  }
  if (order.activation_token_id || order.license_key_plain) {
    if (order.license_key_plain && !order.token_email_sent_at && !order.token_email_error) {
      try {
        await sendLicenceEmail(config, { email: order.buyer_email, name: order.buyer_name, orderReference: order.order_reference, licenceKey: order.license_key_plain });
        await supabase.from("wfilemanager_pro_orders").update({ status: "activation_sent", token_email_sent_at: new Date().toISOString(), token_email_error: null }).eq("id", order.id);
      } catch (error) {
        await supabase.from("wfilemanager_pro_orders").update({ status: "email_failed", token_email_error: error instanceof Error ? error.message : "Email failed" }).eq("id", order.id);
      }
    }
    return;
  }
  ...
  try {
    await sendLicenceEmail(config, { email: order.buyer_email, name: order.buyer_name, orderReference: order.order_reference, licenceKey });
    await supabase.from("wfilemanager_pro_orders").update({ status: "activation_sent", token_email_sent_at: new Date().toISOString(), token_email_error: null }).eq("id", order.id);
  } catch (error) {
    await supabase.from("wfilemanager_pro_orders").update({ status: "email_failed", token_email_error: error instanceof Error ? error.message : "Email failed" }).eq("id", order.id);
  }
...
async function applyRenewal(config: Config, order: Order) {
  ...
  const { data: instance, error } = await supabase.from("wfilemanager_instances").select("id,instance_key,paid_until").eq("instance_key", instanceKey).maybeSingle();
  if (error) throw error;
  if (!instance) throw new Error("Renewal target instance was not found");
  const base = instance.paid_until && new Date(instance.paid_until).getTime() > Date.now() ? new Date(instance.paid_until) : new Date();
  const paidUntil = new Date(base.getTime() + Number(order.period_days || config.periodDays) * 24 * 60 * 60 * 1000).toISOString();
  ...
  try {
    await sendRenewalEmail(config, { email: order.buyer_email, name: order.buyer_name, orderReference: order.order_reference, instanceKey, paidUntil });
    await supabase.from("wfilemanager_pro_orders").update({ status: "renewal_applied", renewal_applied_at: new Date().toISOString(), renewal_paid_until: paidUntil, token_email_sent_at: new Date().toISOString(), token_email_error: null }).eq("id", order.id);
  } catch (error) {
    await supabase.from("wfilemanager_pro_orders").update({ status: "email_failed", renewal_applied_at: new Date().toISOString(), renewal_paid_until: paidUntil, token_email_error: error instanceof Error ? error.message : "Email failed" }).eq("id", order.id);
  }
}
...
async function instanceFor(order: Order, token: TokenRow | null) {
  ...
  if (claimedId) {
    const { data } = await supabase.from("wfilemanager_instances").select("instance_key,paid_until,subscription_status,data_status,status").eq("id", claimedId).maybeSingle();
    if (data) return data;
  }
  if (instanceKey) {
    const { data } = await supabase.from("wfilemanager_instances").select("instance_key,paid_until,subscription_status,data_status,status").eq("instance_key", instanceKey).maybeSingle();
    if (data) return data;
  }
  return null;
}
async function publicOrder(order: Order) {
  const { data: fresh, error } = await supabase.from("wfilemanager_pro_orders").select("*, wfilemanager_pro_activation_tokens(status,claimed_at,expires_at,instance_key,claimed_by_instance_id)").eq("id", order.id).single();
  if (error) throw error;
  ...
  return {
    ...
    keyInstanceKey: instance?.instance_key || token?.instance_key || fresh.target_instance_key || null,
...
async function orderStatus(config: Config, url: URL) {
  ...
  const { data: found, error } = await supabase.from("wfilemanager_pro_orders").select("*").eq("order_reference", reference).maybeSingle();
  if (error) throw error;
  if (!found || String(found.buyer_email).toLowerCase() !== email) return json({ error: "Order not found" }, 404);
  let order = await refreshFromCamerPay(config, found);
  if (String(order.status) === "paid") {
    if (String(order.order_type || "new_licence_key") === "renewal") await applyRenewal(config, order);
    else await issueLicenceKeyOnce(config, order);
    const reload = await supabase.from("wfilemanager_pro_orders").select("*").eq("id", order.id).single();
...
async function webhook(_config: Config, request: Request, rawBody: string) {
  ...
  if (!reference) return json({ success: true, ignored: true, reason: "missing_invoice_reference" });
  ...
  await supabase.from("wfilemanager_pro_orders").update({ webhook_payload: payload, provider_reference: providerRefFrom(payload) || order.provider_reference }).eq("id", order.id);
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "checkout") {
      ...
      return await checkout(config, await request.json().catch(() => ({})) as Record<string, unknown>);
    }
    if (action === "order") {
    ...
    if (action === "status") return json({ ok: true, webhookRequired: false, licenceKeys: true, renewals: true });
