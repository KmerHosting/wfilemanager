...
async function audit(request: Request, params: {
  ...
  await db.from("wfilemanager_audit_logs").insert({
    ...
    metadata: params.reason ? { reason: params.reason, rate_limited_endpoint: true } : { rate_limited_endpoint: true },
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (!instanceKey || !login || !password) return json({ error: "Login and password are required" }, 400);

    const identifier = `${instanceKey}:${login}`;
    const rate = await rateCheck(identifier, ipAddress);
    if (rate.allowed === false) {
      return json({ error: "Too many sign-in attempts. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
    }

    const { data: instance, error: instanceError } = await db.from("wfilemanager_instances")
      .select("*").eq("instance_key", instanceKey).maybeSingle();
    if (instanceError) throw instanceError;
    if (!instance) return json({ error: "Instance is not configured" }, 404);
    if (instance.status === "frozen") return json({
      error: "This installation is frozen after 30 days without a valid server heartbeat. Recover it with the saved Recovery Kit.",
      status: "frozen", deleteAfterAt: instance.delete_after_at,
    }, 423);
    if (instance.subscription_status === "suspended" || instance.subscription_status === "expired" || instance.data_status === "suspended" || instance.data_status === "pending_delete") {
      return json({ error: "This Pro subscription requires payment before sign-in.", paymentRequired: true, deleteAfterAt: instance.delete_after_at }, 402);
    }
    if (instance.status !== "active") return json({ error: "This installation is disabled" }, 403);

    if (instance.service_plan === "pro") {
      const { data: used } = await db.rpc("wfilemanager_refresh_storage_usage", { target_instance_id: instance.id });
      if (typeof used === "number") instance.storage_used_bytes = used;
      if (storageFull(instance)) return json({
        error: "Managed storage is full. Increase the Pro quota before signing in.",
        code: "pro_storage_full", storageUsedBytes: instance.storage_used_bytes,
        storageQuotaBytes: instance.storage_quota_bytes,
      }, 402);
    }

    const { data: user, error: userError } = await db.from("wfilemanager_users").select("*")
      .eq("instance_id", instance.id).or(`username.eq.${login},email.eq.${login}`).maybeSingle();
    if (userError) throw userError;
    if (!user || user.status !== "active") {
      await rateRecord(identifier, ipAddress, false);
      await audit(request, { instanceId: instance.id, username: login, result: "failure", reason: "invalid_credentials" });
    ...
    if (hash !== user.password_hash) {
      await rateRecord(identifier, ipAddress, false);
      await audit(request, { instanceId: instance.id, userId: user.id, username: user.username, result: "failure", reason: "invalid_credentials" });
      return json({ error: "Invalid username or password" }, 401);
    }

    await rateRecord(identifier, ipAddress, true);
    const rawToken = randomHex(32);
    const expiresAt = new Date(Date.now() + (body.remember ? 30 * 24 : 12) * 60 * 60 * 1000).toISOString();
    ...
    await Promise.all([
      ...
      db.from("wfilemanager_instances").update({ last_seen_at: now, updated_at: now }).eq("id", instance.id),
    ]);
    await audit(request, { instanceId: instance.id, userId: user.id, username: user.username, result: "success" });
