...
async function passwordValid(password: string, customer: Customer) {
  const result = await passwordHash(password, customer.password_salt, Number(customer.password_iterations || 150000));
...
function clientIp(request: Request) {
  return (request.headers.get("x-forwarded-for") || request.headers.get("cf-connecting-ip") || "").split(",")[0].trim();
}
...
function paymentUrl(payload: Record<string, unknown>) {
  return clean(pick(payload, ["pay_url","payUrl","payment_url","paymentUrl","checkout_url","checkoutUrl","redirect_url","redirectUrl","url","data.pay_url","data.payment_url","data.checkout_url","data.url"]));
}
function providerReference(payload: Record<string, unknown>) {
  return clean(pick(payload, ["transaction_uuid","uuid","reference","transaction_id","transactionId","payment_id","data.transaction_uuid","data.uuid","data.reference","data.transaction_id"]));
}
...
function providerAmount(payload: Record<string, unknown>) {
  const number = Number(pick(payload, ["amount","paid_amount","data.amount","data.paid_amount"]));
...
async function loadConfig(): Promise<Config> {
  const { data, error } = await db.from("wfilemanager_pro_subscription_config").select("*").eq("id", true).maybeSingle();
...
async function licenceEmail(config: Config, customer: Customer, orderReference: string, key: string) {
...
async function renewalEmail(config: Config, customer: Customer, instanceKey: string, paidUntil: string, balance: number) {
  const name = customer.full_name || "Customer";
  await sendMail(config, {
    email: customer.email,
    name,
...
async function topupEmail(config: Config, customer: Customer, amount: number, balance: number, topupReference: string) {
  const name = customer.full_name || "Customer";
  await sendMail(config, {
    email: customer.email,
    name,
...
async function authenticate(request: Request) {
  ...
  await db.from("wfilemanager_customer_sessions").update({ last_seen_at: new Date().toISOString() }).eq("id", data.id);
...
function validateProfile(input: ReturnType<typeof profile>, requirePassword = false) {
  ...
  if (requirePassword) { const error = passwordPolicy(input.password); if (error) return error; }
...
async function register(request: Request, body: Record<string, unknown>) {
  ...
  if (rate.allowed === false) return json({ error: "Too many account creation attempts. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
  const password = await passwordHash(input.password);
  const { data: customer, error } = await db.from("wfilemanager_customer_accounts").insert({
    email: input.email,
  ...
  return json({ customer: publicCustomer(customer), token: session.token, expiresAt: session.expiresAt }, 201);
}
async function login(request: Request, body: Record<string, unknown>) {
  ...
  if (rate.allowed === false) return json({ error: "Too many sign-in attempts. Try again later.", retryAfterSeconds: rate.retryAfterSeconds }, 429);
  const { data: customer, error } = await db.from("wfilemanager_customer_accounts").select("*").eq("email", email).maybeSingle();
  if (error) throw error;
  ...
  await db.from("wfilemanager_customer_accounts").update({ last_login_at: new Date().toISOString() }).eq("id", customer.id);
  const session = await createSession(customer.id, request);
...
async function logout(request: Request) {
  ...
  if (current) await db.from("wfilemanager_customer_sessions").update({ revoked_at: new Date().toISOString() }).eq("id", current.sessionId);
  return json({ ok: true });
}
async function updateProfile(request: Request, body: Record<string, unknown>) {
  ...
  const { data, error } = await db.from("wfilemanager_customer_accounts").update(update).eq("id", current.customer.id).select("*").single();
  if (error) throw error;
...
async function sessions(request: Request) {
  ...
  if (request.method === "GET") {
    const { data, error } = await db.from("wfilemanager_customer_sessions")
      ...
      .eq("customer_id", current.customer.id).is("revoked_at", null)
      .gt("expires_at", new Date().toISOString()).order("last_seen_at", { ascending: false });
    if (error) throw error;
    return json({ sessions: (data || []).map((item) => ({ ...item, current: item.id === current.sessionId })) });
  }
  const body = await request.json().catch(() => ({})) as Record<string, unknown>;
  if (body.all === true) {
    await db.from("wfilemanager_customer_sessions").update({ revoked_at: new Date().toISOString() }).eq("customer_id", current.customer.id).is("revoked_at", null);
    return json({ success: true, currentRevoked: true });
  }
  const id = clean(body.id);
  if (!id) return json({ error: "Session id is required" }, 400);
  const { error } = await db.from("wfilemanager_customer_sessions").update({ revoked_at: new Date().toISOString() })
    .eq("id", id).eq("customer_id", current.customer.id);
  if (error) throw error;
...
async function instanceFor(order: any, token: any) {
  ...
  if (id) {
    const { data } = await db.from("wfilemanager_instances").select("instance_key,paid_until,subscription_status,data_status,status,auto_renew").eq("id", id).maybeSingle();
    if (data) return data;
  }
  const instanceKey = clean(order.target_instance_key || token?.instance_key);
  if (!instanceKey) return null;
  const { data } = await db.from("wfilemanager_instances").select("instance_key,paid_until,subscription_status,data_status,status,auto_renew").eq("instance_key", instanceKey).maybeSingle();
  return data;
}
async function safeOrder(order: any) {
  ...
  else if (order.order_type === "renewal") keyStatus = order.status === "renewal_applied" ? "renewed" : order.status;
  ...
  return {
    ...
    keyInstanceKey: instance?.instance_key || token?.instance_key || order.target_instance_key || null,
    ...
    canRenew: Boolean((token?.claimed_at || instance?.paid_until) && (instance?.instance_key || token?.instance_key)),
...
async function dashboard(request: Request) {
  ...
  const { data: customer, error: customerError } = await db.from("wfilemanager_customer_accounts").select("*").eq("id", current.customer.id).single();
  if (customerError) throw customerError;
  await db.from("wfilemanager_pro_orders").update({ customer_id: customer.id }).eq("buyer_email", customer.email).is("customer_id", null);
  const { data: orders, error } = await db.from("wfilemanager_pro_orders")
    .select("order_reference,order_type,target_instance_key,status,amount_usd,provider,provider_payment_url,paid_at,token_email_sent_at,token_email_error,license_key_plain,renewal_paid_until,created_at,updated_at,wfilemanager_pro_activation_tokens(status,claimed_at,expires_at,instance_key,claimed_by_instance_id)")
    .eq("buyer_email", customer.email).neq("status", "cancelled").order("created_at", { ascending: false }).limit(50);
  if (error) throw error;
  const safeOrders = [];
  ...
  const { data: topups, error: topupError } = await db.from("wfilemanager_wallet_topups")
    .select("topup_reference,status,amount_usd,provider_payment_url,paid_at,credited_at,email_error,created_at")
    .eq("customer_id", customer.id).order("created_at", { ascending: false }).limit(20);
  if (topupError) throw topupError;
  const { data: transactions, error: transactionError } = await db.from("wfilemanager_wallet_transactions")
  ...
  return json({
    ...
    wallet: { balanceUsd: money(customer.balance_usd), currency: "USD", autoRenewDefault: customer.auto_renew_default === true },
    plan: { name: "wFileManager Pro", priceUsd: config.priceUsd, currency: "USD", periodDays: config.periodDays, storageQuotaBytes: config.storageQuotaBytes },
    orders: safeOrders,
    topups: (topups || []).map((item: any) => ({ reference: item.topup_reference, status: item.status, amountUsd: money(item.amount_usd), paymentUrl: item.provider_payment_url, paidAt: item.paid_at, creditedAt: item.credited_at, emailError: Boolean(item.email_error), createdAt: item.created_at })),
    transactions: (transactions || []).map((item: any) => ({ id: item.id, type: item.transaction_type, amountUsd: money(item.amount_usd), balanceAfterUsd: money(item.balance_after_usd), reference: item.reference, createdAt: item.created_at })),
  });
}
async function linkOrder(customer: Customer, orderReference: string) {
  if (orderReference) await db.from("wfilemanager_pro_orders").update({ customer_id: customer.id }).eq("order_reference", orderReference).eq("buyer_email", customer.email);
}
async function directCheckout(customer: Customer, body: Record<string, unknown>) {
  ...
  if (response.ok && payload.orderReference) await linkOrder(customer, String(payload.orderReference));
...
async function walletBuy(customer: Customer) {
  ...
  if (error) {
    if (String(error.message).includes("insufficient_balance")) return json({ error: `Insufficient balance. $${config.priceUsd.toFixed(2)} USD is required.` }, 402);
    throw error;
  }
  const result = data?.[0];
  try {
    ...
    await db.from("wfilemanager_pro_orders").update({ status: "email_failed", token_email_error: value instanceof Error ? value.message : "Email failed" }).eq("id", result.order_id);
  }
  return json({ success: true, paymentMethod: "balance", orderReference: result.order_reference, licenceKey: result.licence_key, balanceUsd: money(result.balance_usd) });
}
async function checkout(request: Request) {
  const current = await authenticate(request);
  if (!current) return json({ error: "Authentication required" }, 401);
  const body = await request.json().catch(() => ({})) as Record<string, unknown>;
  return clean(body.paymentMode) === "balance" ? walletBuy(current.customer) : directCheckout(current.customer, { ...body, orderType: "new_licence_key" });
}
async function ownedInstance(customer: Customer, instanceKey: string) {
  const { data: token } = await db.from("wfilemanager_pro_activation_tokens").select("id")
    .eq("instance_key", instanceKey).eq("status", "claimed")
    .or(`customer_id.eq.${customer.id},customer_email.eq.${customer.email}`).limit(1).maybeSingle();
  if (!token) return null;
  const { data, error } = await db.from("wfilemanager_instances").select("*").eq("instance_key", instanceKey).maybeSingle();
  if (error) throw error;
  return data;
}
async function walletRenew(customer: Customer, instanceKey: string) {
  ...
  await db.from("wfilemanager_instances").update({ billing_customer_id: customer.id }).eq("id", instance.id);
  const orderReference = reference("WFM-REN-WAL");
  const { data: order, error: orderError } = await db.from("wfilemanager_pro_orders").insert({
    order_reference: orderReference, order_type: "renewal", target_instance_key: instanceKey,
    status: "pending", customer_id: customer.id, buyer_name: customer.full_name, buyer_email: customer.email,
    buyer_phone: customer.phone || "", buyer_company: customer.company, buyer_country: customer.country || "",
    billing_address: customer.billing_address || "", billing_city: customer.billing_city,
    billing_postal_code: customer.billing_postal_code, amount_usd: config.priceUsd,
    amount_xaf: Math.max(1, Math.round(config.priceUsd * config.usdToXafRate)), currency: "USD",
    period_days: config.periodDays, storage_quota_bytes: config.storageQuotaBytes,
    provider: "wallet", payment_source: "wallet", automation_note: "Manual renewal paid from customer USD balance",
  }).select("id").single();
  ...
  try {
    ...
    await db.from("wfilemanager_pro_orders").update({ status: "renewal_applied", paid_at: new Date().toISOString(), renewal_applied_at: new Date().toISOString(), renewal_paid_until: result.paid_until, token_email_sent_at: new Date().toISOString(), token_email_error: null }).eq("id", order.id);
  } catch (value) {
    await db.from("wfilemanager_pro_orders").update({ status: "renewal_applied", paid_at: new Date().toISOString(), renewal_applied_at: new Date().toISOString(), renewal_paid_until: result.paid_until, token_email_error: value instanceof Error ? value.message : "Email failed" }).eq("id", order.id);
  }
  return json({ success: true, paymentMethod: "balance", status: "renewal_applied", orderReference, paidUntil: result.paid_until, balanceUsd: money(result.balance_usd) });
}
async function renew(request: Request) {
  ...
  return clean(body.paymentMode) === "balance" ? walletRenew(current.customer, instanceKey) : directCheckout(current.customer, { orderType: "renewal", targetInstanceKey: instanceKey });
}
async function autoRenew(request: Request) {
  ...
  const { error } = await db.from("wfilemanager_instances").update({
    billing_customer_id: current.customer.id, auto_renew: enabled,
    auto_renew_last_error: null, updated_at: new Date().toISOString(),
  }).eq("id", instance.id);
  if (error) throw error;
  return json({ success: true, instanceKey, autoRenew: enabled });
}
async function createPayment(config: Config, customer: Customer, paymentReference: string, amountXaf: number) {
  ...
  if (["orange_money","mtn_momo","stripe","paypal"].includes(config.camerpayMethod.toLowerCase())) body.payment_method = config.camerpayMethod.toLowerCase();
...
async function topup(request: Request) {
  ...
  if (amountUsd < 5 || amountUsd > 5000) return json({ error: "Top-up amount must be between $5.00 and $5,000.00 USD" }, 400);
  const config = await loadConfig();
  const topupReference = reference("WFM-TOPUP");
  const amountXaf = Math.max(1, Math.round(amountUsd * config.usdToXafRate));
  const { data: row, error } = await db.from("wfilemanager_wallet_topups").insert({
    customer_id: current.customer.id, topup_reference: topupReference, status: "pending",
    amount_usd: amountUsd, amount_xaf: amountXaf, currency: "USD", provider_currency: "XAF",
    exchange_rate: config.usdToXafRate,
  }).select("*").single();
  if (error) throw error;
  try {
    const payment = await createPayment(config, current.customer, topupReference, amountXaf);
    await db.from("wfilemanager_wallet_topups").update({
      status: "payment_pending", provider_reference: payment.providerReference,
      provider_payment_url: payment.url, provider_payload: payment.payload,
      next_reconcile_at: new Date(Date.now() + 5 * 60000).toISOString(), updated_at: new Date().toISOString(),
    }).eq("id", row.id);
    return json({ reference: topupReference, status: "payment_pending", amountUsd, paymentUrl: payment.url });
  } catch (value) {
    await db.from("wfilemanager_wallet_topups").update({ status: "failed", reconciliation_error: value instanceof Error ? value.message : "Payment failed" }).eq("id", row.id);
    throw value;
  }
}
async function topupStatus(request: Request, url: URL) {
  ...
  const config = await loadConfig();
  const { data: found, error } = await db.from("wfilemanager_wallet_topups").select("*").eq("customer_id", current.customer.id).eq("topup_reference", topupReference).maybeSingle();
  if (error) throw error;
  if (!found) return json({ error: "Top-up not found" }, 404);
  let row = found;
  if (!["credited","failed","cancelled"].includes(row.status) && row.provider_reference) {
    const response = await fetch(`${config.camerpayBaseUrl}/api/payment/${row.provider_reference}/status`, { headers: { Authorization: `Bearer ${config.camerpayToken}`, Accept: "application/json" } });
    const payload = await response.json().catch(() => ({})) as Record<string, unknown>;
    if (response.ok && isPaid(providerStatus(payload))) {
      ...
      if (amount === null || amount >= Number(row.amount_xaf)) {
        ...
        await db.from("wfilemanager_wallet_topups").update({ status: "paid", paid_at: paidAt, status_payload: payload }).eq("id", row.id);
  ...
  if (row.status === "paid" && !row.credited_at) {
    ...
    try {
      await topupEmail(config, current.customer, money(row.amount_usd), money(result.balance_usd), row.topup_reference);
      await db.from("wfilemanager_wallet_topups").update({ email_sent_at: new Date().toISOString(), email_error: null }).eq("id", row.id);
    } catch (value) {
      await db.from("wfilemanager_wallet_topups").update({ email_error: value instanceof Error ? value.message : "Email failed" }).eq("id", row.id);
    }
  ...
  const { data: customer } = await db.from("wfilemanager_customer_accounts").select("balance_usd").eq("id", current.customer.id).single();
  return json({ reference: row.topup_reference, status: row.status, amountUsd: money(row.amount_usd), paymentUrl: row.provider_payment_url, balanceUsd: money(customer?.balance_usd) });
}
async function orderStatus(request: Request, url: URL) {
  ...
  const config = await loadConfig();
  const response = await fetch(`${config.subscriptionApi}/order?${new URLSearchParams({ orderReference, email: current.customer.email })}`);
  const payload = await response.json().catch(() => ({}));
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "register" && request.method === "POST") return register(request, await request.json().catch(() => ({})));
    if (action === "login" && request.method === "POST") return login(request, await request.json().catch(() => ({})));
    if (action === "logout" && request.method === "POST") return logout(request);
    if (action === "profile" && ["POST","PUT"].includes(request.method)) return updateProfile(request, await request.json().catch(() => ({})));
    if ((action === "dashboard" || action === "me") && request.method === "GET") return dashboard(request);
    if (action === "sessions" && ["GET","DELETE"].includes(request.method)) return sessions(request);
    ...
    if (action === "status") return json({
      ok: true, customerAccounts: true, secureSessions: true, rateLimiting: true,
      licenceKeys: true, renewals: true, walletUsd: true, topups: true,
      autoRenewOptIn: true, passwordPolicy: "12+ characters",
    });
