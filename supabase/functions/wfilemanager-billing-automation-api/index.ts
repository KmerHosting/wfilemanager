...
async function customerById(id: string): Promise<Customer | null> {
  const { data, error } = await db.from("wfilemanager_customer_accounts")
    .select("id,email,full_name,balance_usd,status").eq("id", id).maybeSingle();
  if (error) throw error;
  if (!data || data.status !== "active") return null;
  return { id: data.id, email: data.email, name: data.full_name || "Customer", balanceUsd: money(data.balance_usd) };
}
async function customerForInstance(instance: Record<string, unknown>): Promise<Customer | null> {
  ...
  const { data: token } = await db.from("wfilemanager_pro_activation_tokens")
    .select("customer_id,customer_email,order_reference")
    .eq("claimed_by_instance_id", instance.id).order("claimed_at", { ascending: false }).limit(1).maybeSingle();
  if (token?.customer_id) return customerById(String(token.customer_id));
  if (token?.order_reference) {
    const { data: order } = await db.from("wfilemanager_pro_orders").select("customer_id").eq("order_reference", token.order_reference).maybeSingle();
...
async function reconcileOrders(config: Config) {
  ...
  const { data: orders, error } = await db.from("wfilemanager_pro_orders")
    ...
    .lte("next_reconcile_at", now).order("created_at", { ascending: true }).limit(100);
  if (error) throw error;
  const results: unknown[] = [];
  for (const order of orders || []) {
    const attempts = Number(order.reconciliation_attempts || 0) + 1;
    try {
      if (["pending","payment_pending"].includes(order.status) && new Date(order.created_at).getTime() < Date.now() - 48 * 3600000) {
        await db.from("wfilemanager_pro_orders").update({
          status: "cancelled", reconciliation_attempts: attempts, last_reconciled_at: now,
          next_reconcile_at: new Date(Date.now() + 365 * 86400000).toISOString(), reconciliation_error: null,
        }).eq("id", order.id);
      ...
      if (!response.ok) throw new Error(clean(payload.error) || `Order reconciliation failed (${response.status})`);
      const terminal = ["activation_sent","renewal_applied","upgrade_applied","failed","cancelled"].includes(clean(payload.status));
      await db.from("wfilemanager_pro_orders").update({
        reconciliation_attempts: attempts,
        last_reconciled_at: new Date().toISOString(),
        next_reconcile_at: new Date(Date.now() + (terminal ? 365 * 86400000 : 5 * 60000)).toISOString(),
...
async function sendTopupConfirmation(config: Config, customer: Customer, topup: Record<string, unknown>, balance: number) {
...
async function reconcileTopups(config: Config) {
  const { data: topups, error } = await db.from("wfilemanager_wallet_topups").select("*")
    .in("status", ["pending","payment_pending","paid","credited"])
    .lte("next_reconcile_at", new Date().toISOString()).order("created_at", { ascending: true }).limit(100);
  if (error) throw error;
  const results: unknown[] = [];
  for (let topup of topups || []) {
    ...
    try {
      ...
      if (["pending","payment_pending"].includes(topup.status) && new Date(topup.created_at).getTime() < Date.now() - 48 * 3600000) {
        await db.from("wfilemanager_wallet_topups").update({ status: "cancelled" }).eq("id", topup.id);
        topup = { ...topup, status: "cancelled" };
      } else if (["pending","payment_pending"].includes(topup.status) && topup.provider_reference) {
        const payload = await checkPayment(config, topup.provider_reference);
        const amount = providerAmount(payload);
        if (paidStatus(providerStatus(payload)) && (amount === null || amount >= Number(topup.amount_xaf || 0))) {
          const paidAt = topup.paid_at || new Date().toISOString();
          await db.from("wfilemanager_wallet_topups").update({ status: "paid", paid_at: paidAt, status_payload: payload }).eq("id", topup.id);
      ...
      if (topup.status === "paid" && !topup.credited_at) {
        ...
        await db.from("wfilemanager_wallet_topups").update({
          status: "credited", credited_at: creditedAt, wallet_transaction_id: applied.transaction_id,
        }).eq("id", topup.id);
        topup = { ...topup, status: "credited", credited_at: creditedAt, wallet_transaction_id: applied.transaction_id };
      }
      if (topup.status === "credited" && (!topup.email_sent_at || topup.email_error)) {
        ...
        await db.from("wfilemanager_wallet_topups").update({ email_sent_at: new Date().toISOString(), email_error: null }).eq("id", topup.id);
      }
      const terminal = ["credited","failed","cancelled"].includes(topup.status);
      await db.from("wfilemanager_wallet_topups").update({
        reconciliation_attempts: attempts, last_reconciled_at: new Date().toISOString(),
        next_reconcile_at: new Date(Date.now() + (terminal ? 365 * 86400000 : 5 * 60000)).toISOString(),
        reconciliation_error: null,
      }).eq("id", topup.id);
...
async function reserveReminder(instanceKey: string, customer: Customer, kind: string, paidUntil: string | null) {
  const { data, error } = await db.from("wfilemanager_billing_reminders").insert({
    instance_key: instanceKey, customer_email: customer.email, reminder_kind: kind, paid_until: paidUntil,
  }).select("id").single();
  if (error) {
...
async function reminderMail(config: Config, instance: Record<string, unknown>, customer: Customer, kind: string) {
  ...
  const paidUntil = instance.paid_until ? new Date(String(instance.paid_until)).toUTCString() : "not set";
  await sendMail(config, {
    email: customer.email, name: customer.name, subject,
...
function reminderKind(instance: Record<string, unknown>) {
  ...
  const deleteAfter = instance.delete_after_at ? (new Date(String(instance.delete_after_at)).getTime() - Date.now()) / 86400000 : null;
...
async function runReminders(config: Config) {
  ...
  const { data: instances, error } = await db.from("wfilemanager_instances").select("*")
    .eq("service_plan", "pro").or(`paid_until.lte.${horizon},delete_after_at.lte.${horizon}`)
    .neq("data_status", "deleted").limit(500);
  if (error) throw error;
  const results: unknown[] = [];
  for (const instance of instances || []) {
    ...
    try {
      ...
      await db.from("wfilemanager_billing_reminders").update({
        email_error: message, next_attempt_at: new Date(Date.now() + 15 * 60000).toISOString(), last_attempt_at: new Date().toISOString(),
      }).eq("id", id);
...
async function runAutoRenew(config: Config) {
  ...
  const { data: instances, error } = await db.from("wfilemanager_instances")
    .select("*").eq("service_plan", "pro").eq("auto_renew", true)
    .not("billing_customer_id", "is", null).not("paid_until", "is", null)
    .lte("paid_until", horizon).neq("data_status", "deleted").limit(500);
  if (error) throw error;
  const results: unknown[] = [];
  for (const instance of instances || []) {
    const customer = await customerForInstance(instance);
    ...
    if (renewalError) {
      ...
      await db.from("wfilemanager_instances").update({
        auto_renew_last_attempt_at: new Date().toISOString(), auto_renew_last_error: message,
      }).eq("id", instance.id);
      if (message.includes("insufficient_balance")) {
        const id = await reserveReminder(instance.instance_key, customer, "auto_renew_insufficient", oldPaidUntil);
        if (id) {
          try {
            ...
            await db.from("wfilemanager_billing_reminders").update({ email_error: mailError instanceof Error ? mailError.message : "Email failed" }).eq("id", id);
          }
        }
      }
    ...
    const id = await reserveReminder(instance.instance_key, updatedCustomer, "auto_renew_success", oldPaidUntil);
    if (id) {
      try {
        await sendMail(config, {
          email: updatedCustomer.email, name: updatedCustomer.name,
...
async function processLifecycle(config: Config) {
  const { data: events, error } = await db.from("wfilemanager_lifecycle_events").select("*")
  ...
  for (const event of events || []) {
    ...
    try {
      await db.from("wfilemanager_lifecycle_events").update({ status: "processing", attempt_count: attempts }).eq("id", event.id);
      const { data: snapshots } = await db.from("wfilemanager_backup_snapshots").select("storage_path").eq("instance_id", event.instance_id);
      ...
      const { error: deletionError } = await db.rpc("wfilemanager_delete_instance", { p_instance_id: event.instance_id });
      if (deletionError) throw deletionError;
      if (event.customer_email) {
        await sendMail(config, {
          email: event.customer_email, name: event.customer_name || "Customer",
      ...
      await db.from("wfilemanager_lifecycle_events").update({
      ...
      await db.from("wfilemanager_lifecycle_events").update({
        ...
        next_attempt_at: new Date(Date.now() + backoffMinutes(attempts) * 60000).toISOString(), last_error: message,
      }).eq("id", event.id);
...
async function runFast(config: Config) {
  const [orders, topups, lifecycle] = await Promise.all([
    reconcileOrders(config), reconcileTopups(config), processLifecycle(config),
  ]);
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (action === "status") return json({
      ok: true, protectedAutomation: true, paymentReconciliation: true,
      walletAutoRenew: true, reminders: true, queuedDeletion: true, currency: "USD",
    });
    const config = await loadConfig();
    if (!await authorized(request, config)) return json({ error: "Unauthorized automation request" }, 401);
